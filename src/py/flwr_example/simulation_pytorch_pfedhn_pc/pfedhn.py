...
class pFedHN_PC(pFedHN):
    """Configurable pFedHN-PC strategy implementation."""
    ...
    def __init__(
        ...
        *,
        ...
        """Personalized Federated Learning using Hypernetworks - pFedHN-PC strategy
        interface.
        Implementation based on https://arxiv.org/pdf/2103.04628.pdf
        Parameters
        ----------
        hypernetwork : Hypernetwork
            The hypernetwork used in order to generate the next weights for a given client
        personal_layers : int
            The number of layers that should be left out from aggregation in order to
            be personalized.
        fraction_evaluate : float, optional
            Fraction of clients used during validation. Defaults to 1.0.
        min_fit_clients : int, optional
        ...
        self.pc_layers = personal_layers

    def __repr__(self) -> str:
        rep = f"pFedHN-PC(accept_failures={self.accept_failures})"
        return rep

    def configure_fit(
            self, server_round: int, parameters: Parameters, client_manager: ClientManager
    ) -> List[Tuple[ClientProxy, FitIns]]:
        params_change = parameters.tensors[:-self.pc_layers]
        pc_layers = parameters.tensors[-self.pc_layers:]
        changed_params = Parameters(tensors=params_change,
                                    tensor_type=parameters.tensor_type)
        [(client, fit_ins)] = super().configure_fit(server_round=server_round,
                                                    parameters=changed_params,
                                                    client_manager=client_manager)
        full_params_tensors = fit_ins.parameters.tensors + pc_layers
        full_params = Parameters(tensors=full_params_tensors,
                                 tensor_type=fit_ins.parameters.tensor_type)
        fit_ins = FitIns(full_params, fit_ins.config)
        return [(client, fit_ins)]
    ...
    def aggregate_fit(
        ...
        """Obtain fit results of network personalized with hypernetworks"""
        [(client, fit_res)] = results
        parameters = fit_res.parameters.tensors[:-self.pc_layers]
        pc_layers = fit_res.parameters.tensors[-self.pc_layers:]
        fit_res.parameters = Parameters(tensors=parameters,
                                        tensor_type=fit_res.parameters.tensor_type)
        results = [(client, fit_res)]
        parameters, metrics = super().aggregate_fit(server_round, results, failures)
        full_parameters = parameters.tensors + pc_layers
        return Parameters(tensors=full_parameters, tensor_type=parameters.tensor_type),\
               metrics
